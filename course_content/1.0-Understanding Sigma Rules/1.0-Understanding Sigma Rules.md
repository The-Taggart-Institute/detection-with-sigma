...
Historically the problem of detection engineering was addressed in silos. The detection engineers would have access to only their logs and those logs are often unique to their etwork. Different tools and platforms use different field names, collection methods, and query methods. Because of these individual silos, detection engineers struggled to share their queries and detection rules with the broader community or to bring those detections into their workplace. With this problem in mind ae of prominent detection engineers, Thomas Patzke and Florian Roth, developed the technology to support Sigma rules. 
...
In order to implement Sigma rules in one’s own omn, they must implement a backend and build pipelines that are responsible for transforming the generic rules into a query language that is specific to their log structure. During this course, we will implement the open source Splunk backend and build our own transformation pipelines. We will ingest rules from the Sigma community, transform them from the generic standard into our own specification, and then use these rules to detect malicious activities in our t.

The Sigma community prefers to use the field names from the Windows security monitoring tool, Sysmon. Throughout this course we will mostly focus on Windows log sources as they represent the bulk of the publicly available Sigma rules. Take a look at the following rule:
